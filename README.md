...
3. **Environment variables** (Settings → Environment Variables): add `VITE_GEMINI_API_KEY` and `VITE_DID_API_KEY`, then **Redeploy** so Vite embeds them at build time. The `.env` file is not committed. **About the Maker → Listen** uses **Microsoft Azure Speech** (Ava multilingual neural) — add `VITE_AZURE_SPEECH_KEY` and `VITE_AZURE_SPEECH_REGION`, then redeploy.
